...
//      List<EventDTO> eventDTO =  userEvents.stream().map(EventDTO::mapToEventDTO).collect(Collectors.toList());
    ...
    public ResponseEntity<EventDTO> createEvent(@Valid @RequestBody EventDTO event) throws BadRequestException {
        ...
        User user = UserAuthenticationUtil.getUserByAuthentication();
        // CORRECTED: Removed the extra user.getId() argument
    ...
    @PutMapping("/{id}")
    public ResponseEntity<?> updateEvent(@PathVariable("id") Long eventId, @Valid @RequestBody EventDTO eventDetails) throws BadRequestException {
        log.info("Updating event with ID {}", eventId);
        try {
            User user = UserAuthenticationUtil.getUserByAuthentication();
            // CORRECTED: Removed the extra user.getId() argument
            Event updatedEvent = eventService.updateEvent(eventId, eventDetails, user);
...
//            EventDTO eventDTO = EventDTO.mapToEventDTO(updatedEvent);
            ...
            return ResponseEntity.status(HttpStatus.NOT_FOUND).body("Event not found for update with ID: " + eventId);
        }
    }
    ...
    public ResponseEntity<String> deleteEvent(@PathVariable("id") Long eventId) {
        ...
        try {
            ...
            return ResponseEntity.status(HttpStatus.OK).body("Event deleted successfully with ID: " + eventId);
        } catch (ResourceNotFoundException ex) {
